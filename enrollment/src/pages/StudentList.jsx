...
import SelectYearBtn from '../components/SelectYearBtn'
...
function StudentList() {
    ...
    const [numsearch, numSearch] = useState("");
    const [secsearch, secSearch] = useState("");

    const displayedStudents = useMemo(() => {
        if (!numsearch && !secsearch) return students;
        else if (numsearch) return students.filter(s => String(s.student_number) === numsearch);
        else return students.filter(s => s.section.toLowerCase().includes(secsearch.toLowerCase()));
    }, [students, numsearch, secsearch]);
                ...
                <section className="flex flex-col md:bg-[url('/logo.png')] bg-no-repeat bg-center bg-contain mt-4 m-6 sm:mt-12 md:m-12 gap-4">
                    ...
                    <div>
                        {loading ? (
                            ...
                            <StudentsTable students={displayedStudents} className="fixed-header" />
                        )}
                    </div>
                </section>
